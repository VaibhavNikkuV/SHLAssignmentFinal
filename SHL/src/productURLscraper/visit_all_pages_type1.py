...
if __name__ == "__main__":
    ...
    urls_type1 = visit_all_catalog_pages(base_url, type_id=1)

    # Create DataFrames for each type
    df_type1 = pd.DataFrame({'Product URLs': urls_type1})

    # Write to Excel with separate sheets
    output_file = "product_catalog_urls_type1.xlsx"
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        df_type1.to_excel(writer, sheet_name='Type 1 Products', index=False)
